...
# Visualization Page
    ...
    # Add interactive selection for variables
    all_variables = df.columns[:-1]  # All columns except the target variable
    selected_variable = st.multiselect("Select variables:", all_variables, default=selected_features)

    # Filter the correlation matrix based on selected variables
    corr_matrix_filtered = corr_matrix.loc[selected_variable, selected_variable]
    ...
    # Plot the heatmap with interactive sorting and thresholding
    fig, ax = plt.subplots()
    heatmap = sns.heatmap(corr_matrix_filtered, annot=True, cmap=cmap, fmt=".2f", ax=ax)

    # Add tooltips for values
    for i in range(len(corr_matrix_filtered)):
        for j in range(len(corr_matrix_filtered.columns)):
            text = f"{corr_matrix_filtered.iloc[i, j]:.2f}"
            ax.text(j + 0.5, i + 0.5, text, ha='center', va='center', color='black', fontsize=10, fontweight='bold')  # Enhance text appearance
    ...
    if index_box < len(df.columns[:-1]):
        ...
        sns.boxplot(data=df, x=df.columns[index_box], ax=ax_box)
        ax_box.set_xlabel(df.columns[index_box])
        ax_box.set_title(f'Box Plot of {df.columns[index_box]}')
        plt.xticks(rotation=45)
        plt.tight_layout()
        ...
        if st.checkbox('Sort Values', key=sort_values_checkbox_key):
            ...
            if sort_order_box == "ascending":
                df_sorted_box = df.sort_values(by=df.columns[index_box])
            else:
                df_sorted_box = df.sort_values(by=df.columns[index_box], ascending=False)
            st.dataframe(df_sorted_box)

        # Dynamic Thresholding: Allow users to adjust threshold for displaying box plot
        min_val = float(df[df.columns[index_box]].min())  # Cast min value to float
        max_val = float(df[df.columns[index_box]].max())  # Cast max value to float
        mean_val = df[df.columns[index_box]].mean()  # No need to cast mean value, it's already float
...
# Conclusion Page
    ...
    st.markdown("<p style='font-family: Arial; font-size: 24px; font-weight: bold; color: white;'>🎮 Kahoot Quiz! 🎮</p>", unsafe_allow_html=True)
